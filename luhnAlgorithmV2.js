...
//Loop through every other digit from right to left and each digit OR
//Loop through every other digit, starting from second last digit to right
const loopAndOrMultiply = (numArr, multiplication) => {
  let arr = [];
...
const check = (accountNumber) => {
  ...
  let mulitpliedNumbers    = loopAndOrMultiply(digits, true);           //yes multiply
  let nonmultipliedNumbers = loopAndOrMultiply(digits, false);       //no don't multiply
